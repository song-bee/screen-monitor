...
class TestPILScreenCaptureProvider:
    ...
    def setup_method(self):
        """Setup for each test"""
        self.config = {"capture_quality": 85, "exclude_menu_bar": True}
    ...
    async def test_macos_menu_bar_exclusion(self):
        ...
        with (
            patch("asam.core.capture.screen.ImageGrab") as mock_grab,
...
class TestMacOSScreenCaptureProvider:
    ...
    @patch(
        "asam.core.capture.screen.MacOSScreenCaptureProvider._check_cocoa_availability",
        return_value=True,
    )
...
class TestScreenCaptureManager:
    ...
    def test_initialization_on_macos(self, mock_platform):
        ...
        with (
            patch("asam.core.capture.screen.MacOSScreenCaptureProvider") as mock_macos,
