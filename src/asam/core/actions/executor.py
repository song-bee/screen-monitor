...
class ScreenLockExecutor(ActionExecutor):
    ...
    async def execute(self, action: ActionType, result: AggregatedResult) -> bool:
        ...
        now = datetime.now()
        if self.last_lock_time and now - self.last_lock_time < timedelta(
            seconds=self.min_lock_interval
        ):
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"Screen locked due to {result.primary_category.value} content "
...
class NotificationExecutor(ActionExecutor):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.min_notification_interval = config.get(
            "min_notification_interval_seconds", 10
        )
    ...
    async def execute(self, action: ActionType, result: AggregatedResult) -> bool:
        ...
        if (
            ...
            and now - self.last_notification_time
            < timedelta(seconds=self.min_notification_interval)
        ):
            return True
    ...
    def _create_notification_message(
        self, action: ActionType, result: AggregatedResult
    ) -> str:
    ...
    async def _send_notification(
        self, title: str, message: str, action: ActionType
    ) -> bool:
...
class ActionExecutionManager:
    ...
    def get_recent_actions(
        self, hours: int = 1
    ) -> list[tuple[datetime, ActionType, AggregatedResult]]:
        """Get actions from the last N hours"""
        cutoff = datetime.now() - timedelta(hours=hours)
        return [
            (timestamp, action, result)
            for timestamp, action, result in self.action_history
            if timestamp > cutoff
        ]
    ...
    def get_action_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "blocks": len(
                [a for _, action, _ in recent_actions if action == ActionType.BLOCK]
            ),
            "warnings": len(
                [a for _, action, _ in recent_actions if action == ActionType.WARN]
            ),
            "allows": len(
                [a for _, action, _ in recent_actions if action == ActionType.ALLOW]
            ),
