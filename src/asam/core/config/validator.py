...
class DetectionConfig(BaseModel):
    ...
    confidence_threshold: float = Field(0.75, ge=0.0, le=1.0, description="Overall confidence threshold for actions")
    analysis_interval_seconds: float = Field(5.0, gt=0.0, description="Time between analysis cycles")
    max_concurrent_analyses: int = Field(3, ge=1, le=10, description="Maximum concurrent analysis tasks")
...
class TextDetectionConfig(BaseModel):
    ...
    llm_model: str = Field("llama3.2:3b", description="LLM model name for text analysis")
    ollama_host: str = Field("http://localhost:11434", description="Ollama server URL")
    max_text_length: int = Field(8000, ge=100, le=50000, description="Maximum text length to analyze")
    timeout_seconds: float = Field(10.0, gt=0.0, description="LLM request timeout")
    weight: float = Field(0.4, ge=0.0, le=1.0, description="Weight in aggregated scoring")
...
class VisionDetectionConfig(BaseModel):
    ...
    motion_threshold: float = Field(6.0, ge=0.0, description="Motion detection sensitivity")
    ad_detection_threshold: float = Field(0.7, ge=0.0, le=1.0, description="Advertisement detection threshold")
    gaming_ui_threshold: float = Field(0.8, ge=0.0, le=1.0, description="Gaming UI detection threshold")
    weight: float = Field(0.3, ge=0.0, le=1.0, description="Weight in aggregated scoring")
...
class ProcessDetectionConfig(BaseModel):
    ...
    check_interval_seconds: float = Field(2.0, gt=0.0, description="Process check interval")
    cpu_threshold: float = Field(50.0, ge=0.0, le=100.0, description="CPU usage threshold for gaming detection")
    weight: float = Field(0.2, ge=0.0, le=1.0, description="Weight in aggregated scoring")
    gaming_processes: list[str] = Field(default_factory=list, description="Known gaming process patterns")
    entertainment_processes: list[str] = Field(default_factory=list, description="Known entertainment process patterns")
...
class NetworkDetectionConfig(BaseModel):
    ...
    check_interval_seconds: float = Field(3.0, gt=0.0, description="Network check interval")
    weight: float = Field(0.1, ge=0.0, le=1.0, description="Weight in aggregated scoring")
    gaming_domains: list[str] = Field(default_factory=list, description="Known gaming domains")
    streaming_domains: list[str] = Field(default_factory=list, description="Known streaming domains")
    social_media_domains: list[str] = Field(default_factory=list, description="Known social media domains")
...
class ScreenCaptureConfig(BaseModel):
    ...
    capture_quality: int = Field(85, ge=1, le=100, description="Screenshot quality (1-100)")
    exclude_menu_bar: bool = Field(True, description="Exclude menu bar from captures (macOS)")
    max_capture_size: tuple[int, int] = Field((1920, 1080), description="Maximum capture dimensions")
    
    @validator('max_capture_size')
    def validate_capture_size(cls, v):
        if len(v) != 2 or any(dim <= 0 for dim in v):
            raise ValueError("max_capture_size must be (width, height) with positive values")
        return v
...
class ActionConfig(BaseModel):
    ...
    min_lock_interval_seconds: int = Field(30, ge=1, description="Minimum time between locks")
    min_notification_interval_seconds: int = Field(10, ge=1, description="Minimum time between notifications")
    detailed_logging: bool = Field(True, description="Enable detailed action logging")
    action_history_size: int = Field(100, ge=1, le=1000, description="Maximum action history entries")
...
class LoggingConfig(BaseModel):
    ...
    max_log_size_mb: int = Field(10, ge=1, le=100, description="Maximum log file size in MB")
...
class SecurityConfig(BaseModel):
    ...
    file_integrity_check: bool = Field(True, description="Enable file integrity monitoring")
    process_monitoring: bool = Field(True, description="Enable process monitoring")
    require_admin_privileges: bool = Field(False, description="Require admin privileges to run")
...
class AsamConfig(BaseModel):
    ...
    process_detection: ProcessDetectionConfig = Field(default_factory=ProcessDetectionConfig)
    ...
    @validator('text_detection', 'vision_detection', 'process_detection', 'network_detection')
    def validate_weights_sum_reasonable(cls, v, values):
        """Validate that analyzer weights sum to a reasonable value"""
        weights = []
        for field_name in ['text_detection', 'vision_detection', 'process_detection', 'network_detection']:
        ...
        if weights and abs(sum(weights) - 1.0) > 0.1:
            logger.warning(f"Analyzer weights sum to {sum(weights):.2f}, consider adjusting for optimal results")
...
class ConfigValidator:
    ...
    def validate_config(self, config_data: dict[str, Any]) -> tuple[AsamConfig, list[str]]:
        """
    ...
    def _validate_analyzer_weights(self, config: AsamConfig) -> list[str]:
        ...
        total_weight = sum(analyzer.weight for _, analyzer in analyzers if analyzer.enabled)
        if total_weight == 0:
            warnings.append("Total analyzer weights sum to 0 - detection may not work properly")
        elif abs(total_weight - 1.0) > 0.2:
            warnings.append(f"Analyzer weights sum to {total_weight:.2f}, consider normalizing to 1.0")
