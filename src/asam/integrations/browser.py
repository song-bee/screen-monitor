...
class BrowserExtensionManager:
    ...
    async def _handle_browser_content(self, browser_content: BrowserContent) -> None:
        """Handle new browser content with ultra-verbose logging"""
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        content_length = len(browser_content.text_content)

        # Ultra-verbose browser content logging
        self.logger.info("🌐 ULTRA-VERBOSE BROWSER CONTENT RECEIVED")
        self.logger.info(f"   ⏰ Timestamp: {timestamp}")
        self.logger.info(f"   📄 Title: '{browser_content.title}'")
        self.logger.info(f"   🔗 URL: {browser_content.url}")
        self.logger.info(f"   🏷️  Tab ID: {browser_content.tab_id}")
        self.logger.info(f"   🌐 Browser Type: {browser_content.browser_type}")
        ...
        self.logger.debug(f"   Avg words/line: {len(words)/max(len(lines), 1):.1f}")

        # Enhanced content preview with line numbers
        ...
        if len(preview_lines) < len(lines):
            remaining_lines = len(lines) - len(preview_lines)
            self.logger.debug(f"   ... plus {remaining_lines} more lines")

        # Content type heuristic analysis
        content_lower = browser_content.text_content.lower()
